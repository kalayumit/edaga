...
class OrderController extends Controller {
    ...
    function __construct(OrderContract $contract)
    {
    ...
    function create(Request $req){
        $this->validate($req, [
            'waiter_id' => 'required|string:orders|exists:waiters,id|uuid',
            'total' => 'required',            
            'items' => 'required|array|min:1',
            'items.*.product_id' => 'required|string:orders|exists:products,id|uuid',
    ...
    function status($id, Request $req){
        return $this->_modelRepository->toggleStatus($id, $req);
    }
}
